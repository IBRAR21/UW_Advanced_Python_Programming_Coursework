...
#### list_user_images.py tests ####
    ...
    def test_reconcile_images_no_difference(self):
        ...
        self.assertEqual(result[0], ['000003.png'])
...
def client():
    ...
    to test our code
    '''
    # The client fixture simulates accessing our code through a
    # web browser (that's the 'client')
...
def test_api_users(client):
    ...
    expected_result = b"abc01"
    assert expected_result in result.data

def test_api_images(client):
    result = client.get('/images')
    expected_result = b"abc01"
    assert expected_result in result.data
